...
couplingValues = [ i*xmin/n for i in range(-n,n+1) ]
deltaX = couplingValues[0] - couplingValues[1]

#model_name = "HEL_UFO"
model_name = "SMEFTatNLO"
...
for proc in processes:
    ...
    hists.append(ROOT.TH1F(proc, proc, len(couplingValues), min(couplingValues)*scale-(deltaX/2.)*scale, max(couplingValues)*scale+(deltaX/2.)*scale))
    ...
    for i,cv in enumerate(couplingValues):
        ...
        print cv
        print modified_couplings
        p = Process(process = proc, nEvents=50000, config = config)
        print p.hasXSec(modified_couplings = modified_couplings)
        if p.hasXSec(modified_couplings = modified_couplings): ratio = p.xsec(modified_couplings = modified_couplings)/SM_xsec[proc]
        else: ratio = u_float(0.)
        print p.xsec(modified_couplings = modified_couplings), SM_xsec[proc]
...
latex1.DrawLatex(0.60,0.96,'#bf{%s (13TeV)}'%model_name.replace('_',' ').replace('UFO',''))
